...
These notes apply to the Cursor Cloud Agent VM. Standard commands live in the sections above and in the README/justfile; only non-obvious caveats are captured here.

- Tooling: `uv` and `just` are installed under `~/.local/bin` and put on `PATH` via `~/.local/bin/env` (sourced from `~/.bashrc`/`~/.profile`). Node is v22 here (CI uses v24); dev, `lint`, and `typecheck` all work on v22. The startup update script only refreshes dependencies (`uv sync --frozen --group dev` and `npm --prefix viewer/web ci`).
- Do NOT run `just setup` verbatim: its `pre-commit install` step fails because the Cloud Agent sets git `core.hooksPath`, and `articraft hooks install`/`pre-commit` are not needed to run or test the app. Use the individual commands instead. Run `uv run --frozen articraft init` once per fresh checkout to initialize local workbench/storage state (idempotent).
- Viewer assets are not committed (`data/cache/` is gitignored), so the 3D viewport is empty until you compile records: run `uv run articraft compile-all --target visual` (or `uv run articraft compile data/records/<id>`) before browsing. `just viewer-dev` serves the API on `:8765` and Vite UI on `:5173` (open `:5173`); `just viewer` builds and serves everything on `:8765`.
- Known non-failure: `tests/agent/test_compile_warnings.py::test_non_runtime_error_failure_sanitizes_local_location_lines` fails here only because the repo is checked out at `/workspace`. The sanitizer in `agent/feedback.py` derives the repo name from the checkout directory, but the test hardcodes a path containing `articraft`, so it falls back to a `.../` form. The other smoke tests pass. Do not "fix" this in code.
- 3D viewer WebGL: Chrome runs software WebGL (SwiftShader). The desktop Chrome wrappers (`/usr/local/bin/chrome` and `/usr/local/bin/google-chrome`) need the `--enable-unsafe-swiftshader` flag or WebGL context creation fails and the viewport stays blank; re-add it if a fresh VM lacks it. Even with WebGL working, the on-demand render loop does not always present frames to the on-screen canvas in real time under software GL — force a paint (e.g. click "Save PNG" in the Render panel, or interact) to make the model appear. Rendering itself is correct (snapshots and joint poses render properly).
